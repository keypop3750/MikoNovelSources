...
/**
 * Source for WtrLab (wtr-lab.com)
 */
class WtrLab : NovelSource() {
    
    override val id: Long = 6015L
    override val name: String = "WtrLab"
    override val baseUrl: String = "https://wtr-lab.com"
    override val lang: String = "en"
    override val hasMainPage: Boolean = true
    
    override suspend fun search(query: String, page: Int): List<NovelSearchResult> {
        val url = "$baseUrl/en/search?text=${query.encodeUrl()}"
        val document = getDocument(url)
        
        return document.select("div.search-result-item, div.novel-item").mapNotNull { element ->
            val titleElement = element.selectFirst("a.novel-title, h3 > a") ?: return@mapNotNull null
    ...
    override suspend fun getNovelDetails(url: String): NovelDetails {
        ...
        val coverUrl = fixUrlOrNull(document.selectFirst("div.novel-cover img")?.attr("src"))
        val description = document.selectFirst("div.novel-summary, div.description")?.text()
        val author = document.selectFirst("a[href*=author], span.author")?.text()
        
        val genres = document.select("div.genres a, a.tag").map { it.text() }
        val statusText = document.selectFirst("span.status, div.status")?.text()
    ...
    override suspend fun getChapterContent(chapterUrl: String): String {
        ...
        val content = document.selectFirst("div.chapter-content, article.chapter-body")
