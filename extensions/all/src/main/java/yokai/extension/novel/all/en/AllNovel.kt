 ...
 */
class AllNovel : NovelSource() {
    
    override val id: Long = 6007L
    ...
    override suspend fun search(query: String, page: Int): List<NovelSearchResult> {
        ...
        return document.select("div.col-novel-main > div.list-novel > div.row").mapNotNull { element ->
    ...
    override suspend fun getNovelDetails(url: String): NovelDetails {
        ...
        val description = document.selectFirst("div.desc-text > p")?.text()
        ...
        infoItems.forEach { item ->
            when {
                item.text().contains("Author", ignoreCase = true) -> 
                    author = item.selectFirst("a")?.text()
                item.text().contains("Status", ignoreCase = true) -> 
                    status = parseNovelStatus(item.selectFirst("a")?.text())
                item.text().contains("Genre", ignoreCase = true) -> 
                    genres.addAll(item.select("a").map { it.text() })
            }
        }
    ...
    override suspend fun getChapterContent(chapterUrl: String): String {
        ...
        content?.select("script, ins, div.ads, div[id*=google]")?.remove()
        return content?.html() ?: ""
    }
