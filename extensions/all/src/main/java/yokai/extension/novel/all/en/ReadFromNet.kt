...
/**
 * Source for ReadFromNet (readfrom.net)
 */
class ReadFromNet : NovelSource() {
    
    override val id: Long = 6006L
    override val name: String = "ReadFromNet"
    override val baseUrl: String = "https://readfrom.net"
    ...
    override suspend fun search(query: String, page: Int): List<NovelSearchResult> {
        ...
        return document.select("div.book-item, div.novel-item").mapNotNull { element ->
            val titleElement = element.selectFirst("a.title, h3 > a") ?: return@mapNotNull null
    ...
    override suspend fun getNovelDetails(url: String): NovelDetails {
        ...
        val coverUrl = fixUrlOrNull(document.selectFirst("div.cover img, div.book-img img")?.attr("src"))
        val description = document.selectFirst("div.description, div.summary")?.text()
    ...
    override suspend fun getChapterList(novelUrl: String): List<NovelChapter> {
        ...
        return document.select("div.chapter-list a, ul.list-chapter a").mapIndexed { index, element ->
            NovelChapter(
                name = element.text(),
